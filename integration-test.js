...
async function testApiAuthentication() {
  ...
  try {
    ...
    if (localInvalidAuth.status === 401) {
      ...
      logTest(
        'Local Invalid API Key Rejection',
        'FAIL',
        `Expected 401, got ${localInvalidAuth.status}`
      )
    }
...
async function testCoreBusinessLogic() {
  ...
  try {
    ...
    if (robots.status === 200 && robots.data.success) {
      ...
      const hasValidStructure = robots.data.data.robots.every(
        (robot) => robot.id && robot.name && robot.status && robot.vendor
      )
      logTest(
        'Robots API Structure',
    ...
    if (createResponse.status === 201 && createResponse.data.success) {
      ...
      logTest(
        'Robot Creation',
        'PASS',
        `Created robot: ${createdRobot?.name || createdRobot?.id || 'Unknown'}`
      )
      ...
      if (createdRobot?.id) {
        ...
        if (getResponse.status === 200 && getResponse.data.success) {
          ...
          logTest(
            'Robot Retrieval',
            'PASS',
            `Retrieved robot: ${retrievedRobot?.name || retrievedRobot?.id || 'Unknown'}`
          )
        } else {
    ...
    const telemetryResponse = await makeRequest(
      `${LOCAL_API}/api/v1/telemetry/aggregated?metric=temperature&aggregation=avg&timeWindow=1h&startDate=2025-09-01&endDate=2025-09-30`,
      {
        headers: { 'X-API-Key': 'urfmp_dev_9f8e7d6c5b4a3910efabcdef12345678' },
      }
    )
...
async function testPerformanceBasics() {
  ...
  try {
    ...
    const concurrentPromises = Array.from({ length: 5 }, () => makeRequest(`${LOCAL_API}/health`))
