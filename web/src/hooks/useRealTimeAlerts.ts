...
export function useRealTimeAlerts(): UseRealTimeAlertsReturn {
  ...
  useEffect(() => {
    const convertedAlerts: RealTimeAlert[] = maintenanceAlerts.map(alert => ({
      ...
      severity: alert.severity === 'high' ? 'error' : alert.severity === 'critical' ? 'critical' : 'warning',
  ...
  useEffect(() => {
    ...
    const handleTelemetryUpdate = (data: any) => {
      ...
      // Voltage alerts
      if (telemetryData.voltage?.supply && (telemetryData.voltage.supply < 46 || telemetryData.voltage.supply > 50)) {
        const severity = telemetryData.voltage.supply < 44 || telemetryData.voltage.supply > 52 ? 'error' : 'warning'
        newAlerts.push({
      ...
      if (telemetryData.gpsPosition?.accuracy?.horizontal && telemetryData.gpsPosition.accuracy.horizontal > 10) {
        newAlerts.push({
        ...
        setAlerts(prev => {
          ...
          const filteredPrev = prev.filter(alert =>
            !(alert.robotId === robotId &&
              ...
              newAlerts.some(newAlert => newAlert.type === alert.type && newAlert.title === alert.title))
          )
  ...
  const filterAlerts = (filters: AlertFilters): RealTimeAlert[] => {
    return alerts.filter(alert => {
      ...
      if (filters.timeRange) {
        if (alert.timestamp < filters.timeRange.start || alert.timestamp > filters.timeRange.end) return false
      }
      return true
    })
  }
