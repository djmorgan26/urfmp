...
const pathStatuses = [
  ...
  { id: 'completed', name: 'Completed', description: 'Path execution finished', color: 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400' },
...
export function EditPathModal({ isOpen, onClose, onSuccess, path }: EditPathModalProps) {
  ...
  // Load path data when modal opens or path changes
  useEffect(() => {
    if (path && isOpen) {
      setFormData({
        name: path.name || '',
        description: path.description || '',
        waypoints: path.waypoints ? [...path.waypoints] : [],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Updating path:', pathData)
      toast.success('Path updated successfully!')
      onSuccess?.()
      onClose()
    } catch (error) {
      toast.error('Failed to update path. Please try again.')
    } finally {
      setIsLoading(false)
    }
  }
